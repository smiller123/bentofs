...
mknod:
	err = bento_mknod(dir, entry, mode, 0);
	if (err)
		goto out_dput;
no_open:
...
static int bento_mknod(struct inode *dir, struct dentry *entry, umode_t mode,
		      dev_t rdev)
{
...
static int bento_create(struct inode *dir, struct dentry *entry, umode_t mode,
		       bool excl)
{
	return bento_mknod(dir, entry, mode, 0);
}

static int bento_mkdir(struct inode *dir, struct dentry *entry, umode_t mode)
{
...
static int bento_symlink(struct inode *dir, struct dentry *entry,
			const char *link)
{
	struct bento_conn *fc = get_bento_conn(dir);
...
static int bento_rename2(struct inode *olddir, struct dentry *oldent,
			struct inode *newdir, struct dentry *newent,
			unsigned int flags)
{
	struct bento_conn *fc = get_bento_conn(olddir);
 ...
 */
static int bento_permission(struct inode *inode, int mask)
{
	struct bento_conn *fc = get_bento_conn(inode);
...
static int bento_setattr(struct dentry *entry, struct iattr *attr)
{
	struct inode *inode = d_inode(entry);
	struct bento_conn *fc = get_bento_conn(inode);
...
static int bento_getattr(const struct path *path, struct kstat *stat,
